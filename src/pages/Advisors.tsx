...
import { ArrowRight, User, Brain, BookOpen, Search } from 'lucide-react';
...
const Advisors = () => {
  ...
  const { data: personas = [], isLoading: loadingPersonas } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, name, title, description, avatar, color, tags')
        .eq('is_active', true);
  ...
  const isLoading = loadingPersonas || loadingFrameworks || loadingBooks;
  ...
  const filteredFrameworks = frameworks.filter(f =>
    ...
    f.mental_models?.some((m: string) => m.toLowerCase().includes(q)) ||
  ...
  const filteredBooks = books.filter(b =>
    ...
    b.key_concepts?.some((c: string) => c.toLowerCase().includes(q)) ||
            ...
            <Tabs defaultValue="personas" className="w-full">
              ...
              <TabsContent value="personas">
                ...
                {filteredPersonas.length === 0 ? (
                  <p className="text-center text-muted-foreground py-12">No personas found{q ? ` for "${search}"` : ''}.</p>
                ) : (
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredPersonas.map((persona, index) => (
                      <motion.div key={persona.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.08 }}>
                        <Link to={`/persona/${persona.id}`} className="block group">
                          <div className="glass-card rounded-2xl p-6 h-full transition-all duration-300 hover:border-primary/30 hover:shadow-glow">
                            <div className="flex items-start gap-4 mb-4">
                              <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${persona.color || 'from-purple-500 to-indigo-700'} flex items-center justify-center text-2xl shadow-soft`}>
                                {persona.avatar || '🧠'}
                              </div>
                              <div className="flex-1">
                                <h2 className="text-xl font-serif font-semibold group-hover:text-primary transition-colors">{persona.name}</h2>
              ...
              <TabsContent value="frameworks">
                ...
                {filteredFrameworks.length === 0 ? (
                  <p className="text-center text-muted-foreground py-12">No frameworks found{q ? ` for "${search}"` : ''}.</p>
                ) : (
                  <div className="grid md:grid-cols-2 gap-6">
                    {filteredFrameworks.map((advisor, index) => (
                      <motion.div key={advisor.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                        <Link to={`/chat/${advisor.id}`} className="block group">
                          <div className="glass-card rounded-2xl p-8 h-full transition-all duration-300 hover:border-primary/30 hover:shadow-glow">
                            <div className="flex items-start gap-5 mb-6">
                              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${advisor.color || 'from-blue-500 to-cyan-700'} flex items-center justify-center text-3xl shadow-soft`}>
                                {advisor.icon || '🎯'}
                              </div>
                              <div className="flex-1">
                                <h2 className="text-2xl font-serif font-semibold group-hover:text-primary transition-colors">{advisor.name}</h2>
                                <p className="text-muted-foreground">{advisor.title}</p>
                              </div>
                            </div>
                            <p className="text-secondary-foreground/80 mb-6 leading-relaxed">{advisor.description}</p>
                            <div className="mb-6">
                              <p className="text-sm text-muted-foreground mb-2">Mental Models:</p>
                              <div className="flex flex-wrap gap-2">
                                {advisor.mental_models?.map((model: string) => (
                                  <span key={model} className="text-sm px-3 py-1 rounded-full bg-secondary text-secondary-foreground">{model}</span>
                                ))}
                              </div>
                            </div>
                            <div className="flex items-center text-primary font-medium">
                              Start Conversation <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
                            </div>
                          </div>
                        </Link>
                      </motion.div>
              ...
              <TabsContent value="books">
                ...
                {filteredBooks.length === 0 ? (
                  <p className="text-center text-muted-foreground py-12">No books found{q ? ` for "${search}"` : ''}.</p>
                ) : (
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredBooks.map((book, index) => (
                      <motion.div key={book.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.08 }}>
                        <Link to={`/book/${book.id}`} className="block group">
                          <div className="glass-card rounded-2xl p-6 h-full transition-all duration-300 hover:border-primary/30 hover:shadow-glow">
                            <div className="flex items-start gap-4 mb-4">
                              ...
                              <div className="flex-1">
                                <h2 className="text-xl font-serif font-semibold group-hover:text-primary transition-colors line-clamp-1">{book.title}</h2>
                            ...
                            <div className="flex flex-wrap gap-1.5 mb-4">
                              {book.genres?.slice(0, 2).map((genre: string) => (
                                <span key={genre} className="text-xs px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground">{genre}</span>
                              ))}
                            </div>
                            <div className="flex items-center text-primary text-sm font-medium">
                              Chat with Book <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                            </div>
                          </div>
                        </Link>
                      </motion.div>
